            ...
            <select class="js-choose form-control" name="bab_dosen_id" style="width:100%;font-size:18px">
              ...
              ?>
                ...
                foreach ($babDosen as $i => $v) :
                  if ($v['category_bab'] == 1 && $v['status_semester'] == 1) :


                    if ($i <= $lastBabIndex) :
                ?>
                      <option value="<?= $v['id_bab_dosen'] ?>"><?= $v['bab_name'] ?></option>
                    <?php elseif ($i == $lastBabIndex + 1 && $isAcceptedLastBab == 2) : ?>
                      <option value="<?= $v['id_bab_dosen'] ?>"><?= $v['bab_name'] ?></option>
                    <?php else : ?>
                      <option value="<?= $v['id_bab_dosen'] ?>" disabled><?= $v['bab_name'] ?>- Locked</option>
              <?php endif;
                  endif;
                endforeach;
              }
